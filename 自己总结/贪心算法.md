...
题解：
第i+1步能走的最远距离一定在第i-1步的最远距离~第i步的最远距离之间再走一步的距离。
```cpp
class Solution {
public:
    int jump(vector<int>& nums) {
        ...
        while(now<nums.size()-1){
            int max_s=0;
            for(int i=pre;i<=now && i<nums.size();i++){
...
public:
    ...
    int largestSumAfterKNegations(vector<int>& nums, int k) {
        ...
        if(k%2) sum-=2*abs(nums[nums.size()-1]);
        return sum;
    }
};
```
...
public:
    int largestSumAfterKNegations(vector<int>& nums, int k) {
        ...
        int now=0;
        int sum=0;
        while(now<nums.size() && nums[now]<0 && k--){//k用完、now为+、now用完都退出
            sum-=nums[now];
            now++;
        }
        if(k<=0){//k用完
            ...
            if(now>=nums.size()){//now也用完了，说明数组全是负数
                sum+=(k%2?2:0)*nums[nums.size()-1];
            }
            else{
                if(now==0){//now==0说明数组全是正数
                    sum+=(k%2?-1:1)*nums[0];
                    now++;
                    while(now<nums.size()){
                        sum+=nums[now];
                    ...
                    if(k%2){//k有余且now当前为正，即在正负数交界处，需取一个数为负数
                        if(nums[now-1]+nums[now]>0){
                            sum+=2*nums[now-1];
                        }
                        else{
                            sum-=nums[now];
...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        int res=0;//当前环节消耗
        int sum=0;//总消耗，可以为负，后续补上即可
        int ans=0;//结果返回的出发点
        for(int i=0;i<gas.size();i++){
            res+=gas[i]-cost[i];
            sum+=gas[i]-cost[i];
            if(res<0){//如果这趟从a到b，无法到达，则a-b之间的点也到不了b，所以出发点一定是在b+1之后
                ans=i+1;
                res=0;
            }
        }
        return sum<0?-1:ans;//如果一整圈下来sum能有余，则说明一定有办法能走完一圈，或者说从某个地方出发一定能弥补后续可能出现的不足。
    }
};
```